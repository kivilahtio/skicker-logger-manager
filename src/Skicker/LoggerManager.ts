...
export class LoggerManager {
  ...
  private configurers: {[loggerName: string]: LoggerConfigurer} = {};
  private initedLoggers: {[loggerName: string]: Date} = {};
  ...
  private injectRootLogger(): void {
    (window as any).logger = this.getLogger();
    (window as any).logger.info("root log4javascript.Logger injected to window.logger");
  }
}
